...
fixable_dir="$(run_summary fixable CVE_SCAN_RESULT=failed CVE_FIXABLE_CVES=2)"
...
assert_contains "${fixable_dir}/slack-summary.txt" "type an @-mention of zen"
assert_contains "${fixable_dir}/slack-summary.txt" "- Fix the 2 fixable CVE(s) called out in the Grype report."
...
assert_contains "${onprem_dir}/slack-summary.txt" "type an @-mention of zen"
assert_contains "${onprem_dir}/slack-summary.txt" "- Investigate the onprem-testbed failures (fresh install: failure, upgrade: success)"
assert_contains "${onprem_dir}/slack-summary.txt" "Zen cannot self-start from this report"
