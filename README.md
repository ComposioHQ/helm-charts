...
You can deploy Composio using the provided Python helper script for easier setup and upgrades:

```bash
# Install Knative, generate values.yaml, and install Helm chart (set these first)
export AWS_ACCOUNT_ID=... 
export IMAGE_TAG=... 
export OPENAI_API_KEY=... 
export POSTGRES_URL=... 
export ECR_TOKEN_COMMAND=...
./deploy-composio.py install

# Upgrade existing Helm release (set these first)
export AWS_ACCOUNT_ID=... 
export IMAGE_TAG=... 
export OPENAI_API_KEY=...
./deploy-composio.py upgrade

# Generate values.yaml from template (set these first)
export AWS_ACCOUNT_ID=... 
export IMAGE_TAG=... 
export OPENAI_API_KEY=...
./deploy-composio.py generate-values
```

Set these before running the script. See script header for details.
