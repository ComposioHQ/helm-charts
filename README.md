...
> An automated job will create required databases for composio stack [check-init.sql](./docs/init.sql)

Create Below required secrets.
...
You can reference to [overwrite-value.yaml](./overwrite-values.yaml) for installation. You can find all parameters in [values.yaml](./composio/values.yaml)

> Note: Please replace <DATABASE_HOST> in overwrite-value.yaml. You can also refer to [example.yaml](./example-values.yaml). If your database has TLS enabled kindly check below docs to configure temporal


### Step 2.1⚙️ Configure TLS for Temporal

To enable TLS for Temporal when connecting to a managed database (for example, **AWS RDS**), follow the steps below.

---

#### 2.2. Download the RDS CA Certificate
...
#### 2.3. Create a Kubernetes Secret
...
#### 2.4. Update the Temporal Values File
...
#### 2.5. Mount the Secret in AdminTools
...
### Step 3: Deploy Composio with Helm
```bash
